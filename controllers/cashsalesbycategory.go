...
func GetSalesByCategory(c echo.Context) error {
	log.Println("Received request to fetch cash sales by category")
	...
	// Query sales by category name from the correct table
	var sales []models.Sale
	log.Printf("Querying sales for category: %s", request.CategoryName)
	if err := db.Table("sales_by_cash").Where("category_name = ?", request.CategoryName).Find(&sales).Error; err != nil {
