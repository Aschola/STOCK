...
func AdminMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		role := c.Get("roleName").(string)
		if role != "Admin" {
			log.Println("[AdminMiddleware] Access denied for non-admin user")
			return c.JSON(http.StatusForbidden, map[string]string{"message": "Access denied"})
		}
...
// OrganizationIDMiddleware ensures the organization ID is set in the context for routes other than login/logout.
func OrganizationIDMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
